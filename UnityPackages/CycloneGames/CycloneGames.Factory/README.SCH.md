...
- **对象池**：`ObjectPool<TParam1, TValue>` 线程安全、自动扩缩容。要求 `TValue : IPoolable<TParam1, IMemoryPool>`。
...
public sealed class Bullet : MonoBehaviour, IPoolable<BulletData, IMemoryPool>
{
    ...
    // 可选：逻辑更新可由 Unity Update 驱动，或通过 pool.UpdateActiveItems() 驱动
