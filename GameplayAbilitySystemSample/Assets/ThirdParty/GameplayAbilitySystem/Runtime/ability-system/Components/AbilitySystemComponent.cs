...
namespace AbilitySystem
{
    public class AbilitySystemComponent : MonoBehaviour
    {
        ...
        public List<GameplayTagScriptableObject> InstantTags = new List<GameplayTagScriptableObject>();
        ...
        void ApplyInstantGameplayEffect(GameplayEffectSpec spec)
        {
            ...
            if (spec.GameplayEffect.gameplayEffectTags.AssetTag != null)
            {
                InstantTags.Add(spec.GameplayEffect.gameplayEffectTags.AssetTag);
            }
        }
        ...
        List<GameplayEffectContainer> EffectRemoveList = new List<GameplayEffectContainer>();
        void CleanGameplayEffects()
        {
            ...
            //     x.spec.GameplayEffect.gameplayEffectTags.RemoveGameplayEffectsWithTag?.Length > 0);

            EffectRemoveList.Clear();
            foreach (GameplayEffectContainer geContainerForRemoveCheck in AppliedGameplayEffects)
            {
                if (geContainerForRemoveCheck.spec.GameplayEffect.gameplayEffectTags.RemoveGameplayEffectsWithTag
                        .Length > 0)
                {
                    foreach (var gameplayTagScriptableObject in geContainerForRemoveCheck.spec.GameplayEffect.gameplayEffectTags.RemoveGameplayEffectsWithTag)
                    {
                        foreach (var tag in InstantTags)
                        {
                            if (tag == gameplayTagScriptableObject)
                            {
                                EffectRemoveList.Add(geContainerForRemoveCheck);
                                Debug.Log($"Effect:{geContainerForRemoveCheck.spec.GameplayEffect.name} RemovedBy {gameplayTagScriptableObject.name} ");
                            }
                        }
                    }
                }
            ...
            foreach (var geContainerForRemove in EffectRemoveList)
            {
                AppliedGameplayEffects.Remove(geContainerForRemove);
            }
            InstantTags.Clear();
        }
